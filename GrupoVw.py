...
class GrupoVw:
    ...
    def changue_to_frames(self):
        iframe = self.robot.brownser.find_element_by_xpath(
            '/html/frameset/frame[1]')
        self.robot.brownser.switch_to.frame(iframe)

        iframe_2 = self.robot.brownser.find_element_by_xpath(
            '//*[@id="divframe"]')
        ...
        iframe_4 = self.robot.brownser.find_element_by_xpath(
            '//*[@id="mainApp"]')
        self.robot.brownser.switch_to.frame(iframe_4)

    def changue_frame_menu(self):
        self.changue_to_frames()
        iframe_5 = self.robot.brownser.find_element_by_xpath(
            '//*[@id="HeaderMenu"]')
        self.robot.brownser.switch_to.frame(iframe_5)

    def login(self, user, robot):
        self.robot.brownser.find_element_by_xpath(
            '//*[@id="txtCPFCNPJ"]').send_keys(user)
        sleep(1)
        self.robot.brownser.find_element_by_xpath(
            '//*[@id="txtSenha"]').send_keys(robot)
    ...
    def goto_volkswagen_brasil(self):
        ...
        self.robot.brownser.find_element_by_xpath(
            '*//a[@href="http://www.volkswagen.com.br"]').click()
